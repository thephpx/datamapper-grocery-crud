...
<script>
	$().ready(function(){
		var dt_i = default_per_page;
		var dt_j = 2;
		
		$("div.dataTables_paginate span").html('');
		
		if(dt_current_offset == 0)
		{
		    dt_i = default_per_page;
            dt_j = 2;
		}else{
		    dt_i = dt_current_offset;
		    dt_j = parseInt(dt_current_offset/default_per_page)+1;
		}
		
		var condition = 0;
		
		if(dt_total_results < (dt_current_offset+(default_per_page*3)))
		{
		    condition = dt_total_results;
		}else{
		    condition = (dt_current_offset+(default_per_page*3));
		}
		
		while(dt_i < condition)
		{
			
			if(dt_i > 0){
				$("div.dataTables_paginate span").append('<a tabindex="0" class="fg-button ui-button ui-state-default" href="'+dt_base_paginate_url+'?offset='+dt_i+'">'+dt_j+'</a>');
			}
			else
			{ 
				$("div.dataTables_paginate span").append('<a tabindex="0" class="fg-button ui-button ui-state-default" href="'+dt_base_paginate_url+'/'+'">'+dt_j+'</a>');
			}
			
			dt_i = dt_i + default_per_page;
			dt_j++;			
		}
		
		if((dt_current_offset-default_per_page) > 0 || dt_current_offset > 0){
		$("div.dataTables_paginate a.first").attr('href',dt_base_paginate_url);
		$("div.dataTables_paginate a.first").removeClass('ui-state-disabled');
		}
	
		if((dt_current_offset-default_per_page) >= 0 && dt_current_offset > 0){
		$("div.dataTables_paginate a.previous").attr('href',dt_base_paginate_url+'?offset='+(dt_current_offset-default_per_page));		
		$("div.dataTables_paginate a.previous").removeClass('ui-state-disabled');
		}
		
		if((dt_current_offset+default_per_page) < dt_total_results){
		$("div.dataTables_paginate a.next").attr('href',dt_base_paginate_url+'?offset='+(dt_current_offset+default_per_page));		
		$("div.dataTables_paginate a.next").removeClass('ui-state-disabled');		
		}
		
		if((dt_current_offset+default_per_page) < dt_total_results){
		
		$("div.dataTables_paginate a.last").attr('href',dt_base_paginate_url+'?offset='+(Math.floor(dt_total_results / default_per_page) * default_per_page));
		$("div.dataTables_paginate a.last").removeClass('ui-state-disabled');
		}
	});
</script>